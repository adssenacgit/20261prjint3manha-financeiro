pipeline {
    ...
    stages {
        stage('Verificar Repositório') {
            steps {
                checkout([$class: 'GitSCM', branches: [[name: '*/master']], useRemoteConfigs: [[url: 'https://github.com/adssenacgit/20261_prjint3_manha/financeiro']]])
            }
        }
        ...
        stage('Construir Imagem Docker') {
            steps {
                script {
                    def appName = 'backend-prjint3-financeiro-api'
        ...
        stage('Fazer Deploy') {
            steps {
                script {
                    ...
                    sh "docker run -d --name ${appName} -p 4000:4000 ${imageTag}"
                }
            }
        }
    }
